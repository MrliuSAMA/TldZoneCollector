...
def pull():
	...
	logging.info("fetch root.zone.sig status:%s,%s"  % (Response.status,Response.reason))
	RD = Response.read()
	...
	Response = HttpConn.getresponse()
	logging.info("fetch root.zone status:%s,%s" % (Response.status,Response.reason))
	RD = Response.read()
	HttpConn.close()
...
def check_move(signame,dataname):
	...
	if res1 != None or res2 != None:
		logging.info("zone file %s verified" % dataname)
		...
		logging.info("zone file %s NOT verified" % dataname)
...
def proc():
	...
	while True:
		try:
			...
			logging.info("fetchfile failed reason:%s" % reason)
